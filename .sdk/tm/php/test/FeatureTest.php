...
// InnoCyberAuthentication SDK feature test
//
...
class FtCtrl extends InnoCyberAuthenticationControl
{
...
class FtClient
{
    ...
    private InnoCyberAuthenticationUtility $utility;

    public function __construct(InnoCyberAuthenticationUtility $utility, array $options)
    {
    ...
    public function get_utility(): InnoCyberAuthenticationUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (InnoCyberAuthenticationContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public InnoCyberAuthenticationContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (InnoCyberAuthenticationContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = InnoCyberAuthenticationConfig::make_feature($name);
    ...
    private static function buildUrl(InnoCyberAuthenticationSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof InnoCyberAuthenticationError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new InnoCyberAuthenticationResponse($fetched);
        }
    ...
    private function fail(InnoCyberAuthenticationContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (InnoCyberAuthenticationContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
