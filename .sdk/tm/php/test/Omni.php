...
//    only - a typed InnoCyberAuthenticationContext there would make every ctx
...
final class InnoCyberAuthenticationOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new InnoCyberAuthenticationContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new InnoCyberAuthenticationSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new InnoCyberAuthenticationResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new InnoCyberAuthenticationError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new InnoCyberAuthenticationResponse($clean['response']);
    ...
    public static function err_from_map($m): ?InnoCyberAuthenticationError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new InnoCyberAuthenticationError(is_string($code) ? $code : '', $msg);
    }
