...
module InnoCyberAuthenticationUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = InnoCyberAuthenticationResponse.new({})
    ...
    if fetch_err
      ...
      response = InnoCyberAuthenticationResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = InnoCyberAuthenticationResponse.new(fetched)
    else
