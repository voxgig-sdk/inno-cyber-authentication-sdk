...
boru-driven command-line client **and** interactive REPL for the InnoCyberAuthentication
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/inno-cyber-authentication-cli)
make build

# 2. See usage (words, entities, env vars)
./inno-cyber-authentication-cli --help
...
INNO_CYBER_AUTHENTICATION_BASE=https://api.example.com ./inno-cyber-authentication-cli --help

# 6. No arguments -> interactive REPL
./inno-cyber-authentication-cli
inno-cyber-authentication> /help
inno-cyber-authentication> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/inno-cyber-authentication-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/inno-cyber-authentication-cli --help
   ```
...
Run with no arguments to open a REPL (prompt `inno-cyber-authentication>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/inno-cyber-authentication-cli`. |
...
(`./inno-cyber-authentication-cli <expr>`) and interactively (the REPL), and expressions compose the
