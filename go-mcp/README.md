# inno-cyber-authentication-mcp

[MCP](https://modelcontextprotocol.io) server exposing the InnoCyberAuthentication SDK as
two agent tools — `inno-cyber-authentication_list` and `inno-cyber-authentication_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/inno-cyber-authentication-mcp)
make build

# 2. Provide credentials via the environment
export INNO_CYBER_AUTHENTICATION_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user inno-cyber-authentication \
  -- /absolute/path/to/inno-cyber-authentication-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./inno-cyber-authentication-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// inno-cyber-authentication_list: first page of records
{ "entity": "authentication" }
{ "entity": "authentication", "query": { } }

// inno-cyber-authentication_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/inno-cyber-authentication-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user inno-cyber-authentication \
     -- "$PWD"/dist/*/inno-cyber-authentication-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `inno-cyber-authentication_list` and `inno-cyber-authentication_load` tools now appear
   in new sessions. Ask the agent to *"list authentication using inno-cyber-authentication"*
   and it calls `inno-cyber-authentication_list` with `{"entity":"authentication"}`.
...
```sh
export INNO_CYBER_AUTHENTICATION_APIKEY=sk_live_xxx            # API key
export INNO_CYBER_AUTHENTICATION_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `inno-cyber-authentication_list` tool
...
### Call the `inno-cyber-authentication_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `inno-cyber-authentication_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `inno-cyber-authentication_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `INNO_CYBER_AUTHENTICATION_APIKEY` | API key sent with every request. |
| `INNO_CYBER_AUTHENTICATION_BASE` | Optional override of the API base URL. |
...
```sh
./inno-cyber-authentication-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"inno-cyber-authentication_load","arguments":{"entity":"authentication","query":{"id":1}}}}'
```
