...
func TestAuthenticationEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set INNOCYBERAUTHENTICATION_TEST_AUTHENTICATION_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func authenticationBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("INNOCYBERAUTHENTICATION_TEST_AUTHENTICATION_ENTID")
	...
	if env["INNOCYBERAUTHENTICATION_TEST_LIVE"] == "TRUE" {
		...
		client = sdk.NewInnoCyberAuthenticationSDK(core.ToMapAny(mergedOpts))
	}
