...
import (
	...
	omni "github.com/voxgig-sdk/inno-cyber-authentication-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.InnoCyberAuthenticationError); is {
				out := map[string]any{
					"name":    "InnoCyberAuthenticationError",
...
func omniCtx(arg any, client *sdk.InnoCyberAuthenticationSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.InnoCyberAuthenticationSDK); is {
			client = live
