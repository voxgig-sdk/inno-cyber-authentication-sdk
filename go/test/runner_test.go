...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("INNOCYBERAUTHENTICATION_TEST_LIVE") == "TRUE" ||
		os.Getenv("INNOCYBERAUTHENTICATION_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("INNOCYBERAUTHENTICATION_TEST_EXPLAIN"); explain != "" {
