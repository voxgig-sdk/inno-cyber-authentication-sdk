...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("INNO_CYBER_AUTHENTICATION_TEST_LIVE") == "TRUE" ||
		os.Getenv("INNO_CYBER_AUTHENTICATION_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("INNO_CYBER_AUTHENTICATION_TEST_EXPLAIN"); explain != "" {
...
type entityTestSetup struct {
	client         *sdk.InnoCyberAuthenticationSDK
	data           map[string]any
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.InnoCyberAuthenticationSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.InnoCyberAuthenticationError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.InnoCyberAuthenticationSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.InnoCyberAuthenticationError{Msg: msg, Code: code}
}
