# InnoCyberAuthentication Lua SDK



The Lua SDK for the InnoCyberAuthentication API — an entity-oriented client using Lua conventions.
...
GitHub release tag (`lua/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/inno-cyber-authentication-sdk/releases)),
...
```lua
-- Create
local created, err = client:Authentication():create({ newPassword = "example_newPassword", password = "example_password", referralCode = "example_referralCode", token = "example_token" })
...
```lua
local authentication, err = client:Authentication():create({ newPassword = "example", password = "example", referralCode = "example", token = "example" })
if err then error(err) end
```
...
```
lua/
├── inno-cyber-authentication_sdk.lua    -- Main SDK module
...
The main module (`inno-cyber-authentication_sdk`) exports the SDK constructor
