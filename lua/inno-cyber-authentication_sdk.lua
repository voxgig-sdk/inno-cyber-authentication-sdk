...
require("inno-cyber-authentication_types")
...
local InnoCyberAuthenticationSDK = {}
InnoCyberAuthenticationSDK.__index = InnoCyberAuthenticationSDK
...
InnoCyberAuthenticationSDK._make_feature = _make_feature


function InnoCyberAuthenticationSDK.new(options)
  local self = setmetatable({}, InnoCyberAuthenticationSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function InnoCyberAuthenticationSDK:options_map()
...
function InnoCyberAuthenticationSDK:get_utility()
  return Utility.copy(self._utility)
end


function InnoCyberAuthenticationSDK:get_root_ctx()
  return self._rootctx
end


function InnoCyberAuthenticationSDK:prepare(fetchargs)
...
function InnoCyberAuthenticationSDK:direct(fetchargs)
...
function InnoCyberAuthenticationSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function InnoCyberAuthenticationSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "InnoCyberAuthenticationSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function InnoCyberAuthenticationSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function InnoCyberAuthenticationSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "InnoCyberAuthenticationSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function InnoCyberAuthenticationSDK:Authentication(data)
...
function InnoCyberAuthenticationSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = InnoCyberAuthenticationSDK.new(sdkopts)
  sdk.mode = "test"
...
return InnoCyberAuthenticationSDK
