...
// InnoCyberAuthentication SDK retry feature
...
class InnoCyberAuthenticationRetryFeature extends InnoCyberAuthenticationBaseFeature
{
    ...
    public function init(InnoCyberAuthenticationContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (InnoCyberAuthenticationContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(InnoCyberAuthenticationContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(InnoCyberAuthenticationContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
