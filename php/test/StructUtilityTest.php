...
// InnoCyberAuthentication SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = InnoCyberAuthenticationOmni::makeStructRunner(
    ...
    {
        return InnoCyberAuthenticationOmni::objmodel(InnoCyberAuthenticationOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return InnoCyberAuthenticationOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return InnoCyberAuthenticationOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(InnoCyberAuthenticationOmni::structfix($now, Struct::undef()))
                        !== json_encode(InnoCyberAuthenticationOmni::structfix($val, Struct::undef()))) {
