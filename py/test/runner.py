# InnoCyberAuthentication SDK test runner
...
class InnoCyberAuthenticationTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            InnoCyberAuthenticationTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = InnoCyberAuthenticationTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = InnoCyberAuthenticationTestRunner.getenv("INNO_CYBER_AUTHENTICATION_TEST_LIVE")
        override = InnoCyberAuthenticationTestRunner.getenv("INNO_CYBER_AUTHENTICATION_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = InnoCyberAuthenticationTestRunner.getenv(key)
        ...
        explain = InnoCyberAuthenticationTestRunner.getenv("INNO_CYBER_AUTHENTICATION_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if InnoCyberAuthenticationTestRunner._test_control is not None:
            return InnoCyberAuthenticationTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                InnoCyberAuthenticationTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            InnoCyberAuthenticationTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = InnoCyberAuthenticationTestRunner.load_test_control()
...
def load_env_local():
    InnoCyberAuthenticationTestRunner.load_env_local()


def env_override(m):
    return InnoCyberAuthenticationTestRunner.env_override(m)


def entity_data(v):
    return InnoCyberAuthenticationTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return InnoCyberAuthenticationTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return InnoCyberAuthenticationTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return InnoCyberAuthenticationTestRunner.load_test_control()


def live_client_options():
    return InnoCyberAuthenticationTestRunner.live_client_options()


def live_delay_ms():
    return InnoCyberAuthenticationTestRunner.live_delay_ms()
