...
require_relative 'InnoCyberAuthentication_types'
...
class InnoCyberAuthenticationSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = InnoCyberAuthenticationHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = InnoCyberAuthenticationHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, InnoCyberAuthenticationFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    InnoCyberAuthenticationUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = InnoCyberAuthenticationHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = InnoCyberAuthenticationHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = InnoCyberAuthenticationHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => InnoCyberAuthenticationError.new(
        "#{op}_allow",
        "InnoCyberAuthenticationSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue InnoCyberAuthenticationError => err
    ...
    if fetched.is_a?(Hash)
      status = InnoCyberAuthenticationHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = InnoCyberAuthenticationError.new(
        "graphql_error", "InnoCyberAuthenticationSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = InnoCyberAuthenticationSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
