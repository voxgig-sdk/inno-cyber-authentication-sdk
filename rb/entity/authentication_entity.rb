# InnoCyberAuthentication SDK Authentication entity
...
class AuthenticationEntity
  ...
  def data_set(args)
    if args
      @_data = InnoCyberAuthenticationHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = InnoCyberAuthenticationHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Authentication, Hash] the created Authentication; raises InnoCyberAuthenticationError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        if ctx.result.resdata
          @_data = InnoCyberAuthenticationHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
