...
//     (\`new InnoCyberAuthenticationSDK(...)\` / \`InnoCyberAuthenticationSDK.test(...)\` become
//     \`InnoCyberAuthenticationSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'InnoCyberAuthenticationSDK'
...
// rewrite real client construction (\`new InnoCyberAuthenticationSDK(...)\` and any
// \`InnoCyberAuthenticationSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+InnoCyberAuthenticationSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/InnoCyberAuthenticationSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
